...
assert(customer.includes('resolveCustomerNewArrivals'));assert(customer.includes("typeof profile?.showNewArrivalsOverride==='boolean'"));assert(customer.includes('showNewArrivals=resolveCustomerNewArrivals(window.__customerPortalControl,safeProfile)'));assert(customer.includes("navigator.serviceWorker.register('./customer-sw.js?v=35.0'"),'bootstrap transform marker must remain compatible');assert(boot.includes("runtime/customer-v37-source.txt?v=55.5"));
