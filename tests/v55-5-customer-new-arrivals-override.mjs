...
assert(customer.includes('resolveCustomerNewArrivals'));assert(customer.includes("typeof profile?.showNewArrivalsOverride==='boolean'"));assert(customer.includes('showNewArrivals=resolveCustomerNewArrivals(window.__customerPortalControl,safeProfile)'));assert(customer.includes("navigator.serviceWorker.register('./customer-sw.js?v=35.0'"),'bootstrap transform marker must remain compatible');assert(/runtime\/customer-v37-source\.txt\?v=[0-9.]+/.test(boot),'customer bootstrap must load the versioned customer runtime');
