...
must(runtime.includes("VERSION='56.62'")&&runtime.includes("VISUAL_REVISION='56.62'"),'V56.62 runtime/cache revision missing');
...
for(const imp of [
  '@import url("./v56-49-nd96-home.css?v=56.62")',
  '@import url("./v56-50-nd96-catalog.css?v=56.62")',
  '@import url("./v56-51-nd96-categories.css?v=56.62")',
  '@import url("./v56-52-nd96-cart.css?v=56.62")'
])must(bridge.includes(imp),`V56.62 bridge import missing: ${imp}`);
...
])must(fs.existsSync(asset)&&fs.statSync(asset).size>0,`National Day vector asset missing/empty: ${asset}`);
...
must(home.includes('V56.62 — HOME / INVENTORY'),'Home V56.62 marker missing');
...
must(home.includes('brand-emblem-wide.png'),'Home official badge reference missing');
must(home.includes('--nd96-corner:url("./national-day-96/assets/corner.svg")'),'Home local vector corner missing');
must(home.includes('--nd96-landscape:url("./national-day-96/assets/landscape.svg")'),'Home local vector landscape missing');
...
must(home.includes('.nd96-inventory-hero::before')&&home.includes('var(--nd96-corner)'),'Home restrained corner composition missing');
must(home.includes('.nd96-warehouse-empty::before')&&home.includes('var(--nd96-landscape)'),'Home skyline composition missing');
must(home.includes('.nd96-warehouse-button.bg-primary'),'Home active warehouse treatment missing');

must(catalog.includes('V56.62 — CUSTOMER / CATALOG'),'Catalog V56.62 marker missing');
...
must(catalog.includes('.rights-bar::after')&&catalog.includes('var(--nd96-corner)'),'Catalog header motif missing');
...
must(categories.includes('V56.62 — CATEGORIES'),'Categories V56.62 marker missing');
must(categories.includes('.nd96-categories-page::before')&&categories.includes('var(--nd96-corner)'),'Categories edge motif missing');
...
must(cart.includes('V56.62 — CART'),'Cart V56.62 marker missing');
...
must(cart.includes('.nd96-cart-page::before')&&cart.includes('var(--nd96-corner)'),'Cart top motif missing');
must(cart.includes('.nd96-empty-cart::before')&&cart.includes('var(--nd96-landscape)'),'Cart empty-state skyline missing');
must(cart.includes('.nd96-empty-cart::after')&&cart.includes('var(--nd96-corner)'),'Cart empty-state corner motif missing');
...
console.log('V56.62 National Day Safari-safe four-view + no-store + app-safety regression: PASS');
