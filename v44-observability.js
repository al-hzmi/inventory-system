...
function actor(){
  ...
  if(isCustomerPath){const q=quickProfile();const id=String(q?.uid||visitorId()||installId());return {type:'customer',id,name:String(q?.name||guestName()||'').trim(),company:String(q?.company||'').trim(),visitorId:visitorId(),key:'customer_'+id,surface:'customer_portal'};}
  return {type:'system',id:'unknown',name:'',key:'system_unknown',surface:path};
}
const me=actor();
if(me.type==='system'||isQa()||isQaIdentity(me.name)){window.__V44_OBSERVABILITY={version:VERSION,skipped:true,qa:true};return;}
const DEFAULTS={employee:{search:true,scanner:true,catalog:true,cart:true,drafts:true,orders:true,customerView:true,stocktake:true},customer:{browse:true,search:true,categories:true,images:true,cart:true,checkout:true,orders:true,whatsapp:true,install:true}};
let permissionDoc={};let effective={...DEFAULTS[me.type]};let db=null;let permUnsub=null;let presenceTimer=null;let cartTimer=null;let searchTimer=null;let lastCartHash='';const sessionId='v44s_'+Date.now().toString(36)+'_'+Math.random().toString(36).slice(2,8);const dev=device();
function serverTs(){return window.firebase?.firestore?.FieldValue?.serverTimestamp?.()||new Date();}
function initDb(){try{if(!window.firebase)return null;if(!window.firebase.apps?.length)return null;return window.firebase.firestore()}catch{return null}}
...
function resolvePermissions(){const kind=me.type;const base={...DEFAULTS[kind],...(permissionDoc?.[kind+'Defaults']||{})};const map=permissionDoc?.[kind+'Overrides']||{};for(const key of actorOverrideKeys()){if(map[key])Object.assign(base,map[key])}effective=base;applyPermissionDom();}
...
function denied(){toast('هذه الميزة غير مفعّلة لهذا الحساب. تواصل مع الإدارة عند الحاجة.');}
function classify(el){const t=norm(el?.innerText||el?.textContent||'');const href=String(el?.getAttribute?.('href')||'');const aria=norm(el?.getAttribute?.('aria-label')||el?.getAttribute?.('title')||'');const x=t+' '+aria+' '+href;
  if(me.type==='customer'){
    if(/اضافه للطلب|السله|تعديل التوزيع|ازاله/.test(x))return'cart';
