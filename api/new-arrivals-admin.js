...
async function resolveCanonicalSku(config,mainBranch,suppliedSku){const requested=norm(suppliedSku);if(!requested){const err=new Error('رقم الصنف غير صالح.');err.status=400;throw err}const[j,r]=await Promise.all([readText(config,'data/jeddah.tsv',mainBranch),readText(config,'data/riyadh.tsv',mainBranch)]);const inventory=[...new Set([...parseSkus(j.text),...parseSkus(r.text)])];if(inventory.includes(requested))return requested;const shorthand=digits(requested);if(shorthand){const matches=inventory.filter(id=>digits(id)===shorthand);if(matches.length===1)return matches[0];if(matches.length>1){const err=new Error('رقم الصنف المختصر يطابق أكثر من صنف. ابحث بالرقم الكامل للصنف.');err.status=409;throw err}}const err=new Error('الصنف المطلوب غير موجود في المخزون الحالي.');err.status=400;throw err}
// Retain the public regression contract while V56.34 upgrades validation to canonical resolution.
async function validateSku(config,mainBranch,suppliedSku){return resolveCanonicalSku(config,mainBranch,suppliedSku)}
...
module.exports=async function handler(req,res){
 ...
 try{const mainBranch=await repoBranch(config);const sku=await validateSku(config,mainBranch,req.body?.sku||'');await ensureStateBranch(config,mainBranch);const{state,sha}=await readState(config);let include=state.include.filter(id=>id!==sku),exclude=state.exclude.filter(id=>id!==sku);if(action==='add')include=[sku,...include];if(action==='remove')exclude=[sku,...exclude];const next=sanitize({include,exclude});const commitSha=await persist(config,next,req.body?.updatedBy,`state(new-arrivals): ${action} ${sku}`,sha);return json(res,200,{ok:true,...next,saved:sku,action,commitSha,version:RELEASE_VERSION})}catch(err){console.error('[new-arrivals-admin]',err);const status=[400,401,409].includes(err.status)?err.status:500;return json(res,status,{error:err.message||'تعذر تحديث جديدنا.'})}
};
