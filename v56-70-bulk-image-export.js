...
const S={ready:false,images:new Map,imagesBySku:new Map,digits:new Map,known:new Set,bindings:{},bindingsReady:false,bindingsPromise:null,names:new Map,results:[],missing:[],duplicates:0,invalid:0,overflow:0,busy:false,page:0,shareOffset:0,sharePrepared:null};
...
function resolve(sku){const m=norm(S.bindings?.[sku]||'');if(m){if(S.imagesBySku.has(m))return{...S.imagesBySku.get(m),mode:'يدوي'};if(S.images.has(m))return{...S.images.get(m),mode:'يدوي'}}if(S.imagesBySku.has(sku))return{...S.imagesBySku.get(sku),mode:'مباشر'};if(S.images.has(sku))return{...S.images.get(sku),mode:'مباشر'};if(/^\d+$/.test(sku)){const a=S.digits.get(sku)||[];if(a.length===1)return{...a[0],mode:'مطابقة رقمية'}}return null}
