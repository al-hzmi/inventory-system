...
async function loadBindings(){try{const r=await fetchTimed('./api/image-admin?action=bindings',3500,{cache:'no-store'});if(!r.ok)return{};const d=await r.json();return d?.bindings&&typeof d.bindings==='object'?d.bindings:{}}catch(e){console.warn('[V56.76 bindings skipped]',e?.name||e);return{}}}
...
async function boot(){E.extract.disabled=true;E.extract.textContent='جاري تجهيز بيانات الصور…';startBindings();try{const [i,j,r,p]=await Promise.all([text('./data/images_list.txt'),text('./data/jeddah.tsv'),text('./data/riyadh.tsv'),text('./data/pricing.tsv').catch(()=> '')]);parseImages(i);parseInv(j);parseInv(r);parsePricing(p);S.ready=true;E.extract.disabled=false;E.extract.textContent='استخراج الصور'}catch(e){console.error('[V56.76 boot]',e);E.extract.disabled=false;E.extract.textContent='إعادة المحاولة';E.extract.onclick=()=>location.reload();toast('تعذر تحميل بيانات الصور خلال المهلة. تحقق من الاتصال ثم أعد المحاولة.',true)}}
...
async function run(){if(!S.ready)return toast('بيانات الصور لم تجهز بعد.',true);if(S.bindingsPromise&&!S.bindingsReady)await Promise.race([S.bindingsPromise,wait(700)]);const p=parsed();if(!p.items.length){E.input.focus();return toast('أدخل رقم صنف واحدًا على الأقل.',true)}S.results=[];S.missing=[];S.duplicates=p.duplicates;S.invalid=p.invalid;S.overflow=p.overflow;S.page=0;S.shareOffset=0;S.sharePrepared=null;p.items.forEach((sku,index)=>{const img=resolve(sku),salePrice=S.prices.get(sku)||'',packQty=S.packs.get(sku)||'',unitPrice=unitPriceLabel(salePrice,packQty);img?S.results.push({sku,index,name:S.names.get(sku)||'',salePrice,packQty,unitPrice,...img}):S.missing.push({sku,index,name:S.names.get(sku)||''})});render(p.items.length);toast(p.overflow?'تمت معالجة أول 500 صنف فقط.':'تم العثور على '+S.results.length+' صورة من '+p.items.length+'.',Boolean(p.overflow))}
...
async function prepareShareBatch(){
  ...
  try{
    ...
    console.error('[V56.76 prepare share]',e);
...
async function openPreparedShare(){
  ...
  try{
    ...
    else{console.error('[V56.76 native share]',e);toast('تعذر فتح المشاركة. حاول مرة أخرى أو استخدم ZIP.',true)}
