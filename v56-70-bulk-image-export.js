...
const S={ready:false,images:new Map,digits:new Map,known:new Set,prices:new Map,packs:new Map,bindings:{},bindingsReady:false,bindingsPromise:null,names:new Map,results:[],missing:[],duplicates:0,invalid:0,overflow:0,busy:false,page:0,shareOffset:0,sharePrepared:null};
...
function unitPriceFor(sku,img){const candidates=[sku,img?.key,norm(img?.file||'')].filter(Boolean);for(const key of candidates){const salePrice=S.prices.get(key)||'',packQty=S.packs.get(key)||'',unitPrice=unitPriceLabel(salePrice,packQty);if(unitPrice)return{salePrice,packQty,unitPrice,priceKey:key}}return{salePrice:S.prices.get(sku)||'',packQty:S.packs.get(sku)||'',unitPrice:'',priceKey:''}}
async function loadBindings(){try{const r=await fetchTimed('./api/image-admin?action=bindings',3500,{cache:'no-store'});if(!r.ok)return{};const d=await r.json();return d?.bindings&&typeof d.bindings==='object'?d.bindings:{}}catch(e){console.warn('[V56.77 bindings skipped]',e?.name||e);return{}}}
...
async function boot(){E.extract.disabled=true;E.extract.textContent='جاري تجهيز بيانات الصور…';startBindings();try{const [i,j,r,p]=await Promise.all([text('./data/images_list.txt'),text('./data/jeddah.tsv'),text('./data/riyadh.tsv'),text('./data/pricing.tsv').catch(()=> '')]);parseImages(i);parseInv(j);parseInv(r);parsePricing(p);S.ready=true;E.extract.disabled=false;E.extract.textContent='استخراج الصور'}catch(e){console.error('[V56.77 boot]',e);E.extract.disabled=false;E.extract.textContent='إعادة المحاولة';E.extract.onclick=()=>location.reload();toast('تعذر تحميل بيانات الصور خلال المهلة. تحقق من الاتصال ثم أعد المحاولة.',true)}}
...
function updatePriceCoverage(){
  ...
  const priced=S.results.filter(x=>x.unitPrice).length,unpriced=S.results.length-priced;
  E.priceCoverage.hidden=false;E.priceCoverage.className='bulk-price-coverage'+(unpriced?' warn':'');
  E.priceCoverage.textContent=unpriced?('سعر الحبة جاهز لـ '+priced+' صورة · '+unpriced+' صورة بدون سعر أو شد في بيانات النظام.'):'سعر الحبة متوفر لكل الصور.';
}
...
async function zipAll(){if(S.busy||!S.results.length)return;S.busy=true;E.zip.disabled=true;E.share.disabled=true;E.extract.disabled=true;try{progress(0,S.results.length,'جاري تجهيز ملف واحد لكل الصور…');if(!(await ensureZip()))throw Error('ZIP_LIBRARY');const z=new JSZip();let done=0;for(const x of S.results){z.file(String(x.index+1).padStart(3,'0')+'-'+outputName(x),await outputBlob(x));done++;progress(done,S.results.length,'تجهيز الصور: '+done+' / '+S.results.length);await wait(0)}z.file('_manifest.tsv','SKU\tImage File\tName\tSale Price\tPack\tUnit Price\n'+S.results.map(x=>x.sku+'\t'+x.file+'\t'+(x.name||'')+'\t'+(x.salePrice||'')+'\t'+(x.packQty||'')+'\t'+(x.unitPrice||'')).join('\n'));progress(S.results.length,S.results.length,'إنشاء ملف ZIP الواحد…');const out=await z.generateAsync({type:'blob',compression:'STORE'},m=>{E.progressText.textContent='إنشاء الملف الواحد: '+Math.round(m.percent)+'%'});download(out,priceStampEnabled()?'صور-الأصناف-سعر-الحبة.zip':'صور-الأصناف.zip');toast('تم تجهيز '+S.results.length+' صورة في ملف ZIP واحد.')}catch(e){console.error('[V56.77 single zip]',e);toast('تعذر إكمال التنزيل الكامل. جرّب مرة أخرى أو استخدم المشاركة.',true)}finally{S.busy=false;E.zip.disabled=!S.results.length;E.share.disabled=!S.results.length;E.extract.disabled=false;updateShareButton();setTimeout(()=>{E.progress.classList.remove('on');E.bar.style.width='0'},1200)}}
...
E.input.oninput=count;E.extract.onclick=run;E.paste.onclick=paste;E.clear.onclick=reset;E.fresh.onclick=reset;E.copy.onclick=copyMissing;E.share.onclick=shareImages;E.zip.onclick=zipAll;E.priceToggle.onchange=()=>{S.shareOffset=0;S.sharePrepared=null;renderCards();updateShareButton();toast(E.priceToggle.checked?'سيتم وضع سعر الحبة بختم صغير جدًا أسفل يمين الصور.':'تم إيقاف سعر الحبة على الصور.')};E.input.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key==='Enter')run()});count();boot();
})();