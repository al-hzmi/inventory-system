...
async function zipAll(){if(S.busy||!S.results.length)return;S.busy=true;E.zip.disabled=true;E.share.disabled=true;E.extract.disabled=true;try{progress(0,S.results.length,'جاري تجهيز ملف واحد لكل الصور…');if(!(await ensureZip()))throw Error('ZIP_LIBRARY');const z=new JSZip();let done=0;for(const x of S.results){z.file(String(x.index+1).padStart(3,'0')+'-'+outputName(x),await outputBlob(x));done++;progress(done,S.results.length,'تجهيز الصور: '+done+' / '+S.results.length);await wait(0)}z.file('_manifest.tsv','SKU\tImage File\tName\tPrice\n'+S.results.map(x=>x.sku+'\t'+x.file+'\t'+(x.name||'')+'\t'+(x.price||'')).join('\n'));progress(S.results.length,S.results.length,'إنشاء ملف ZIP الواحد…');const out=await z.generateAsync({type:'blob',compression:'STORE'},m=>{E.progressText.textContent='إنشاء الملف الواحد: '+Math.round(m.percent)+'%'});download(out,priceStampEnabled()?'صور-الأصناف-مع-الأسعار.zip':'صور-الأصناف.zip');toast('تم تجهيز '+S.results.length+' صورة في ملف ZIP واحد.')}catch(e){console.error('[V56.75 single zip]',e);toast('تعذر إكمال التنزيل الكامل. جرّب مرة أخرى أو استخدم المشاركة.',true)}finally{S.busy=false;E.zip.disabled=!S.results.length;E.share.disabled=!S.results.length;E.extract.disabled=false;updateShareButton();setTimeout(()=>{E.progress.classList.remove('on');E.bar.style.width='0'},1200)}}
