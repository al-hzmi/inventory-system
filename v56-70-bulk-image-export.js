...
const S={ready:false,images:new Map,digits:new Map,bindings:{},bindingsReady:false,bindingsPromise:null,names:new Map,results:[],missing:[],duplicates:0,overflow:0,busy:false,page:0,shareOffset:0,sharePrepared:null};
...
async function loadBindings(){try{const r=await fetchTimed('./api/image-admin?action=bindings',3500,{cache:'no-store'});if(!r.ok)return{};const d=await r.json();return d?.bindings&&typeof d.bindings==='object'?d.bindings:{}}catch(e){console.warn('[V56.72 bindings skipped]',e?.name||e);return{}}}
...
async function boot(){E.extract.disabled=true;E.extract.textContent='جاري تجهيز بيانات الصور…';startBindings();try{const [i,j,r]=await Promise.all([text('./data/images_list.txt'),text('./data/jeddah.tsv'),text('./data/riyadh.tsv')]);parseImages(i);parseInv(j);parseInv(r);S.ready=true;E.extract.disabled=false;E.extract.textContent='استخراج الصور'}catch(e){console.error('[V56.72 boot]',e);E.extract.disabled=false;E.extract.textContent='إعادة المحاولة';E.extract.onclick=()=>location.reload();toast('تعذر تحميل بيانات الصور خلال المهلة. تحقق من الاتصال ثم أعد المحاولة.',true)}}
...
async function run(){if(!S.ready)return toast('بيانات الصور لم تجهز بعد.',true);if(S.bindingsPromise&&!S.bindingsReady)await Promise.race([S.bindingsPromise,wait(700)]);const p=parsed();if(!p.items.length){E.input.focus();return toast('أدخل رقم صنف واحدًا على الأقل.',true)}S.results=[];S.missing=[];S.duplicates=p.duplicates;S.overflow=p.overflow;S.page=0;S.shareOffset=0;S.sharePrepared=null;p.items.forEach((sku,index)=>{const img=resolve(sku);img?S.results.push({sku,index,name:S.names.get(sku)||'',...img}):S.missing.push({sku,index,name:S.names.get(sku)||''})});render(p.items.length);toast(p.overflow?'تمت معالجة أول 500 صنف فقط.':'تم العثور على '+S.results.length+' صورة من '+p.items.length+'.',Boolean(p.overflow))}
...
function render(total){E.empty.hidden=true;E.stats.hidden=false;E.toolbar.hidden=false;E.requested.textContent=total;E.found.textContent=S.results.length;E.missingCount.textContent=S.missing.length;E.duplicates.textContent=S.duplicates;E.zip.disabled=!S.results.length;E.copy.disabled=!S.missing.length;E.zip.textContent='ZIP';updateShareButton();E.resultsSection.hidden=!S.results.length;E.missingSection.hidden=!S.missing.length;E.chip.textContent=S.results.length+' صورة';renderCards();E.missingList.innerHTML=S.missing.map(x=>'<span class="bulk-missing-chip">'+esc(x.sku)+'</span>').join('');E.missingHint.textContent=S.missing.length+' رقم';requestAnimationFrame(()=>window.scrollTo({top:Math.max(0,E.stats.offsetTop-100),behavior:'smooth'}))}
...
async function prepareShareBatch(){
  ...
  try{
    ...
    console.error('[V56.72 prepare share]',e);
...
async function openPreparedShare(){
  ...
  try{
    ...
    else{console.error('[V56.72 native share]',e);toast('تعذر فتح المشاركة. حاول مرة أخرى أو استخدم ZIP.',true)}
...
function reset(){if(S.busy)return;E.input.value='';S.results=[];S.missing=[];S.page=0;S.shareOffset=0;S.sharePrepared=null;E.stats.hidden=true;E.toolbar.hidden=true;E.resultsSection.hidden=true;E.missingSection.hidden=true;E.empty.hidden=false;E.grid.innerHTML='';E.pager.innerHTML='';E.missingList.innerHTML='';count();window.scrollTo({top:0,behavior:'smooth'});E.input.focus()}
