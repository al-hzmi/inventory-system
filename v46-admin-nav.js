...
function openSecurity(){
  ...
  let script=document.getElementById('v49-security-loader');if(script)script.remove();
  script=document.createElement('script');script.id='v49-security-loader';script.src='./v48-admin-security.js?v=50.0-'+Date.now();script.async=true;
...
function ensureFallback(){let b=document.getElementById('v49-security-fallback');if(!b){b=document.createElement('button');b.id='v49-security-fallback';b.type='button';b.innerHTML='<span>⌁</span><span>أمان الدخول</span>';b.onclick=openSecurity;document.body.appendChild(b)}return b}
function makeAdminLink(id,href,icon,label){let a=document.getElementById(id);if(!a){a=document.createElement('a');a.id=id;a.href=href;a.className='v45-admin-tab';a.innerHTML=`<span>${icon}</span><span>${label}</span>`;a.style.textDecoration='none'}return a}
function add(){
  installFallbackCss();
  const control=document.getElementById('v46-control-tab'),security=document.getElementById('v49-security-tab'),inventory=document.getElementById('v50-inventory-tab'),fallback=ensureFallback();
  if(!isAdmin()){if(control)control.remove();if(security)security.remove();if(inventory)inventory.remove();fallback.classList.remove('v49-security-visible');return}
  const host=preferredHost();if(!host){fallback.classList.add('v49-security-visible');return}fallback.classList.remove('v49-security-visible');
  const a=control||makeAdminLink('v46-control-tab','./control-center.html','⚙','مركز التحكم');if(a.parentElement!==host)host.appendChild(a);
  const inv=inventory||makeAdminLink('v50-inventory-tab','./inventory-analytics.html','↕','حركة المخزون');if(inv.parentElement!==host)host.appendChild(inv);
  let b=security;if(!b){b=document.createElement('button');b.id='v49-security-tab';b.type='button';b.className='v45-admin-tab';b.innerHTML='<span>⌁</span><span>أمان الدخول</span>';b.onclick=openSecurity}
  if(!b.classList.contains('v45-admin-tab'))b.style.cssText='height:44px;border:1px solid #e7e5e4;border-radius:11px;background:#fff;color:#1c1917;padding:0 12px;font-weight:700;font-size:11px';
...
let tries=0;const timer=setInterval(()=>{add();if(++tries>=40&&document.getElementById('v50-inventory-tab')&&document.getElementById('v49-security-tab'))clearInterval(timer)},250);
